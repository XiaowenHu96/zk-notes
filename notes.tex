...
\begin{enumerate}
    ...
    \item Verifier compute $commit_{\mathbb{G}_2}(Z(x)) =
        ...
        \footnote{Does $P_y$ have to be calculated by the verifier. Can prover
        trick the verifier if they construct $P_y$ instead?}
        \footnote{Here for the frist time we have a commitment under $\mathbb{G}_2$. Is it possible to avoid committing on G2?}
\end{enumerate}
...
We first apply multiple-poly-signle-point method.
...
\begin{description}
    ...
    \item $\prod_i(x-z_i) Q_i(X) = \sum_{i,j} P_{ij}(X) - y_{ij}$
\end{description}
This is our target in the paring function.\\

\begin{description}
    \item For each point $z_i$, prover computes $Q_i(X)$ and $cm_{q} = commit(\sum_i
        Q_i(X))$
    \item Prover also compute $cm_p = \sum(cm_{p_i})$
    \item Verifier compute $P_z = G_2 \sum_i(\tau - z_i)$ and $P_y = \sum_i(y_i G_1)$
\end{description}

\[
\begin{aligned}
    e({cm}_q, P_z) &= e(\sum({cm}_{p_i})-P_y, G_2)\\
    e(\sum\sum_i Q_i\tau^iG_1, G_2 \sum_i(\tau - z_i)) &=
    e(\sum({cm}_{p_i})-\sum_i{(y_i G_1)}, G_2)\\
    e(G_1, \tau G_2)^{\prod Q(\tau) \prod(\tau - z_i)} &= e(\sum\sum_i(p_i\tau^i G_1)
                            -\sum_i{(y_i G_1)}, G_2)\\
    e({cm}_q, \tau G_2 - zG_2) &= e(G1\cdot (\sum\sum_i{(p_i\tau^i)} - \sum_i{y_i}), G_2)\\
    e(G_1, G_2)^{q(\tau)(\tau - z)} &= e(G_1,G_2)^{\sum{(p_i(\tau)-y_i)}}\\
    \mathbb{X}^{q(\tau)(\tau - z)}_{\mathbb{G}_T} &= \mathbb{X}^{\sum{(p_i(\tau)-y_i)}}_{\mathbb{G}_T}
